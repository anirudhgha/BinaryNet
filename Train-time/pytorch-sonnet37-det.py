...
class BinaryTanh(torch.autograd.Function):
    ...
    def forward(ctx, input):
        # hard_sigmoid: clip((x+1)/2, 0, 1)
        y = torch.clamp((input + 1) / 2, 0, 1)
        y = torch.round(y)  # round to 0 or 1
        y = 2 * y - 1       # map to -1 or 1
    ...
    def backward(ctx, grad_output):
        ...
        # Hard tanh gradient for backward pass - zero out gradient outside [-1,1]
        grad_input = grad_input * (input > -1).float() * (input < 1).float() # paper says to cut off at -1 and 1, but theano implementatio does not cut off
        return grad_input
